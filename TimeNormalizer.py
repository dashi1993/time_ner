...
class TimeNormalizer:
    ...
    def parse(self, target, timeBase=arrow.now()):
        ...
        if len(res) == 1 and res[0].period != '':
            tr = {'Y':1,'M':2,'W':3}
            dic['type'] = 'timespan'
            if 'timestamp' in dic.keys():
                del dic['timestamp']
            if tr[res[0].period] == 1:
                dic['timespan'] = [res[0].time.format("YYYY-MM-DD HH:mm:ss"), res[0].time.shift(years=1).format("YYYY-MM-DD HH:mm:ss")]
            if tr[res[0].period] == 2:
                dic['timespan'] = [res[0].time.format("YYYY-MM-DD HH:mm:ss"), res[0].time.shift(months=1).format("YYYY-MM-DD HH:mm:ss")]
            if tr[res[0].period] == 3:
                dic['timespan'] = [res[0].time.format("YYYY-MM-DD HH:mm:ss"), res[0].time.shift(weeks=1).format("YYYY-MM-DD HH:mm:ss")]
    ...
    def __timeEx(self):
        ...
        for i in range(0, rpointer):
            res.append(TimeUnit(temp[i], self, contextTp)) ## mian step
            contextTp = res[i].tp
