...
class TimeUnit:
    ...
    def time_normalization(self):
        ...
        for i in range(0, 4):
            if self.tp.tunit[i] != -1:
                ...
                # print (i)
        # print (flag)
        print (self.normalizer.isTimeSpan)
        if flag:
        ...
        if self.normalizer.isTimeSpan:
            ...
            # print (self.tp.tunit)
            tag = -1
            cur = self.normalizer.timeBase
            print (self.tp.tunit)
            if self.tp.tunit[0] > 0:
                # year = cur.split('-')[0]
                # print (year)
                base = arrow.get(self.normalizer.timeBase,"YYYY-M-D-H-m-s")
                print (base)
                base = base.replace(month=1,day=1,hour=0,minute=0,second=0)
                print (base, self.sign)
                t = self.sign*self.tp.tunit[0]
                print (t,type(t))
                base = base.shift(years=t)
                print ('dfsfsdfds',base)
                self.base = base
                print (self.base)
                tag = 0
            ...
            if self.tp.tunit[1] > 0:
                ...
                print (base)
                base = base.shift(months=self.sign*self.tp.tunit[1])
                self.base = base
                tag = 1
            ...
            if self.tp.tunit[2] > 0:
                if not self.isWeek:
                    ...
                    print (base)
                    print (self.sign)
                    base = base.shift(days=self.sign*self.tp.tunit[2])

                else:
                    week = cur.split('-')[2]
                    base = arrow.get(self.normalizer.timeBase,"YYYY-M-D-H-m-s")
                    base = base.replace(hour=0,minute=0,second=0)
                    base = base.shift(weeks=self.sign*self.tp.tunit[2])
                    ms = base.weekday()
                    base = base.shift(days=-ms)
                self.base = base
                tag = 2
            ...
            self.normalizer.timeSpan = self.genSpan(tag)
            self.period = ''
            return
    ...
    def genSpan(self,tag):
        if tag == -1:
            return
        if tag == 0:
            nowtime = arrow.get(self.normalizer.timeBase,"YYYY-M-D-H-m-s")
            nowtime = nowtime.replace(month=1,day=1,hour=0,minute=0,second=0)
            return [self.base.format("YYYY-MM-DD HH:mm:ss"),nowtime.format("YYYY-MM-DD HH:mm:ss")]
        if tag == 1:
            nowtime = arrow.get(self.normalizer.timeBase,"YYYY-M-D-H-m-s")
            nowtime = nowtime.replace(day=1,hour=0,minute=0,second=0)
            return [self.base.format("YYYY-MM-DD HH:mm:ss"),nowtime.format("YYYY-MM-DD HH:mm:ss")]            
        if tag == 2:
            if self.isWeek:
                nowtime = arrow.get(self.normalizer.timeBase,"YYYY-M-D-H-m-s")
                nowtime = nowtime.replace(hour=0,minute=0,second=0)
                nowtime = nowtime.shift(days=-nowtime.weekday())
                return [self.base.format("YYYY-MM-DD HH:mm:ss"),nowtime.format("YYYY-MM-DD HH:mm:ss")]
            else:
                nowtime = arrow.get(self.normalizer.timeBase,"YYYY-M-D-H-m-s")
                nowtime = nowtime.replace(hour=0,minute=0,second=0)
                return [self.base.format("YYYY-MM-DD HH:mm:ss"),nowtime.format("YYYY-MM-DD HH:mm:ss")]
        # totime = nowtime
        # day = self.sign * days #seconds // (3600*24)
        # # print (day)
        # # print (self.sign)
        # totime = totime.shift(days=day)
        # # h = self.sign * (seconds % (3600*24)) // 3600
        # # m = self.sign * ((seconds % (3600*24)) % 3600) // 60
        # # s = self.sign * ((seconds % (3600*24)) % 3600) % 60
        # #totime.shift(days=day)#,hours=h,minutes=m,seconds=s)
        # # return str(days+day) + ' days, ' + "%d:%02d:%02d" % (h, m, s)
        # if self.sign > 0:
        #     return [nowtime.format("YYYY-MM-DD HH:mm:ss"),totime.format("YYYY-MM-DD HH:mm:ss")]
        # else:
        #     return [totime.format("YYYY-MM-DD HH:mm:ss"),nowtime.format("YYYY-MM-DD HH:mm:ss")]
    ...
    def modifyTimeBase(self):
        ...
        :return:
        """
        # print ('22', self.normalizer.isTimeSpan)
